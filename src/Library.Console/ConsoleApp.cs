...
public class ConsoleApp
{
    ...
    JsonData _jsonData;
        ...
        public ConsoleApp(ILoanService loanService, IPatronService patronService, IPatronRepository patronRepository, ILoanRepository loanRepository, BookService bookService, JsonData jsonData)
        {
    ...
    {
        // Ensure selectedPatronDetails is not null before accessing its properties
        ...
        // Ensure selectedPatronDetails is fetched properly
        if (selectedPatronDetails == null || selectedPatronDetails.Id == 0)
        {
            Console.WriteLine("Invalid patron details.");
            return ConsoleState.PatronSearch;
        }

        selectedPatronDetails = await _patronRepository.GetPatron(selectedPatronDetails.Id) ?? throw new InvalidOperationException("Patron not found.");
    ...
    {
        // Ensure selectedLoanDetails is not null before assigning
        ...
        // Ensure selectedLoanDetails is fetched properly
        if (selectedLoanDetails == null || selectedLoanDetails.Id == 0)
        {
            Console.WriteLine("Invalid loan details.");
            return ConsoleState.PatronSearch;
        }

        selectedLoanDetails = await _loanRepository.GetLoan(selectedLoanDetails.Id) ?? throw new InvalidOperationException("Loan not found.");
    ...
    {
        Console.WriteLine("Checking book availability...");

        // Example: Fetch book items (this would be replaced with actual data fetching logic)
        var bookItems = new List<BookItem>
        {
            new BookItem { Id = 1, IsAvailable = true },
            new BookItem { Id = 2, IsAvailable = false }
        };

        bool isAvailable = _bookService.IsBookAvailable(bookItems);
        Console.WriteLine($"Book is {(isAvailable ? "available" : "not available")}");

        // Simulate an asynchronous operation for BookAvailability
        await Task.Delay(1);
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        {
            Console.WriteLine($"\"{book.Title}\" is on loan to another patron. The return due date is {loan.DueDate:yyyy-MM-dd}.");
        }
        return ConsoleState.PatronSearch;
    }
}
